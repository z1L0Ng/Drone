...
- [ ] Dispatch phase2 multilingual candidate-matrix task to acoustic agent (license/sample-size/semantic-alignment gated).
- [ ] Output artifacts:
...
- 2026-04-07:
  ...
  - Next: wait for `preprocess_ext` completion receipt, then track `branch_trial` startup/completion receipts; in parallel, have acoustic agent prepare multilingual candidate matrix (FR/others allowed) under the same semantic mapping contract.
