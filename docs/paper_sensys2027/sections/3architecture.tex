...
\subsection{Voice as an Interaction Event Source}

The system treats speech as a safety-relevant interaction source for a nearby drone. It is not an open-ended speech assistant and it is not a direct speech-to-flight controller. Its role is to convert short onboard audio windows into intent/state events and expose enough state for a conservative controller to ignore, hold, escalate, or forward the event. Figure~\ref{fig:system_architecture} shows the resulting architecture.
...
\end{cases}
\]
where $g(\cdot)$ maps recognized intent into the bridge-visible state: movement becomes pending interaction, emergency becomes safety-critical handling, and unknown becomes fallback/no-action behavior. The threshold $\tau$ is not the paper's main contribution; it is the guardrail that keeps low-confidence audio from becoming an actionable state.
