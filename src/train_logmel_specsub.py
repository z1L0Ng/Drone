# --- Log-Mel + Spectral Subtraction, SNR -25dB to -10dB ---
...
MODELS_PATH = "saved_models/logmel_specsub/"
RESULT_PATH = "result/logmel_specsub/"
...
ckpt_path = os.path.join(MODELS_PATH, "logmel_specsub_best.weights.h5")
callbacks = [
    ModelCheckpoint(ckpt_path, save_best_only=True, monitor='val_accuracy', save_weights_only=True, verbose=1),
    EarlyStopping(patience=10, restore_best_weights=True, verbose=1)
]

print("🚀 Start Log-Mel + Spectral Subtraction Training...")
...
plt.title('Log-Mel+SpecSub Accuracy')
plt.legend()
plt.subplot(1, 2, 2)
plt.plot(history.history['loss'], label='Train')
plt.plot(history.history['val_loss'], label='Val')
plt.title('Log-Mel+SpecSub Loss')
plt.legend()
...
plt.title('Log-Mel+SpecSub Confusion Matrix')
plt.ylabel('True')
