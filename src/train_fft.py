...
MODELS_PATH = "saved_models/fft/"
RESULT_PATH = "result/fft/"
...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def _extract_features(self, y):
        ...
        # 2) Optional log compression for dynamic range stabilization
        if USE_LOG:
            feat = librosa.amplitude_to_db(mag, ref=np.max, top_db=TOP_DB)
        else:
            feat = mag

        # 3) Pad / crop time frames
...
model = build_model((N_BINS, MAX_FRAMES, 1), NUM_CLASSES)
model.compile(optimizer=Adam(1e-4), loss='categorical_crossentropy', metrics=['accuracy'])

ckpt_path = os.path.join(MODELS_PATH, "fft_best.weights.h5")
callbacks = [
    ModelCheckpoint(ckpt_path, save_best_only=True, monitor='val_accuracy', save_weights_only=True, verbose=1),
    EarlyStopping(patience=10, restore_best_weights=True, verbose=1)
]

print("🚀 Start FFT Training...")
