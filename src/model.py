...
def branchformer_block(x, head_size, num_heads, ff_dim, dropout=0.1, kernel_size=31, block_idx=0):
    ...
    conv_u = Conv1D(filters=input_feature_dim, kernel_size=1, padding='same', name=f"{prefix}_conv_u")(conv_input)
    conv_v = Conv1D(filters=input_feature_dim, kernel_size=1, padding='same', activation='sigmoid', name=f"{prefix}_conv_v")(conv_input)
    ...
    conv_out = Conv1D(filters=input_feature_dim, kernel_size=1, padding='same', name=f"{prefix}_conv_pw2")(conv_dw)
